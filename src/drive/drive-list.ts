...
export function listFiles(
  fileName: string,
  callback: (err: string, message: string) => void
): void {
    ...
    drive.files.list(
      ...
      (err: Error, res: any) => {
        if (err) {
          ...
          res = res["data"]["files"];
          if (res.length == 0) {
            callback(null, "There are no files matching your search query.😢");
          }
          ...
          for (let index = 0; index < res.length; index++) {
            // res[index]['adLink'] = 'test'
            ariaTools.SingleADURL(res[index]["url"], (err: any, res1: any) => {
              res[index]["adLink"] = res1.body;

              if (res.length - 1 == index) {
                console.log("send callback now");
...
function generateFilesListMessage(files: any[]): string {
  ...
  if (files.length > 0) {
    for (var i = 0; i < files.length; i++) {
      message +=
        "<a href = '" + files[i]["adLink"] + "'>" + files[i]["name"] + "</a>";
      if (files[i]["size"])
